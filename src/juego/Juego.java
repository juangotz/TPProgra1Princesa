...
public class Juego extends InterfaceJuego {
	...
	public void tick() {
		...
		if (!gatoRescatado && empezarJuego) {
			...
			if (bart!=null) {
				...
				if (bart.vidas==3) {
					entorno.escribirTexto("Vidas: " + bart.vidas + " (MAX)", 0, 10);
				} else {
					entorno.escribirTexto("Vidas: " + bart.vidas, 0, 10);
				}
				entorno.cambiarFont("Arial", 14, Color.ORANGE);
				entorno.escribirTexto("Puntaje: " + puntaje, 0, 25);
				entorno.cambiarFont("Arial", 14, Color.ORANGE);
				entorno.escribirTexto("Enemigos Asesinados: " + enemigosEliminados, 0, 40);
			}
			if (gato!=null) {
			...
			if (bart==null) {
				...
				entorno.cambiarFont("Arial", 26, Color.ORANGE);
				entorno.escribirTexto("M o r i s t e !", 340, 300);
				entorno.cambiarFont("Arial", 26, Color.WHITE);
	...
	public void reiniciarJuego() { //reiniciar en caso de que muera
		...
		balas = new BalaEnemiga[enemigos.length];// para que los bloques que esten abajo no se puedan romper
		gatoRescatado = false;
		this.puntaje = 0; //inicializo la variable puntaje
	...
	public void refrescarPisos(int contador) {
			for(int i = 0; i < p.length; i++) {
				p[i] = new Piso(120 + i * (entorno.alto() / p.length));
			}
			int contE = 0;
