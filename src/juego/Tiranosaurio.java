...
public boolean detectarApoyo(Tiranosaurio ti, Bloque bl) {
...
public boolean detectarApoyo(Tiranosaurio ti, Piso pi) {
	for(int i = 0; i < pi.bloques.length; i++) {
		if(pi.bloques[i] != null && detectarApoyo(ti, pi.bloques[i])) {
			return true;
		}
	}
...
public boolean detectarApoyo(Tiranosaurio ti, Piso[] pisos) {
	for(int i = 0; i < pisos.length; i++) {
		if(detectarApoyo(ti , pisos[i])) {
			return true;
		}
	}
